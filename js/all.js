...
const getLabels = (labelsData) => {
  const labels = labelsData.map((data) => {
    if (data.toLowerCase() === "bug") {
      return `<span class="bg-red-100 text-red-600 border-red-300 inline-flex items-center gap-0.5 sm:gap-1 md:gap-1.5 lg:gap-2 rounded-full border-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 lg:py-1.5 text-[8px] sm:text-[9px] md:text-xs lg:text-sm font-semibold uppercase tracking-tight"> <i class="fa-solid fa-bug"></i> BUG </span>`;
    }
    if (data.toLowerCase() === "help wanted") {
      return `<span class="bg-yellow-100 text-yellow-600 border-yellow-300 inline-flex items-center gap-0.5 sm:gap-1 md:gap-1.5 lg:gap-2 rounded-full border-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 lg:py-1.5 text-[8px] sm:text-[9px] md:text-xs lg:text-sm font-semibold uppercase tracking-tight"> <i class="fa-regular fa-life-ring"></i> HELP WANTED </span>`;
    }
    if (data.toLowerCase() === "enhancement") {
      return `<span class="bg-green-100 text-green-600 border-green-300 inline-flex items-center gap-0.5 sm:gap-1 md:gap-1.5 lg:gap-2 rounded-full border-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 lg:py-1.5 text-[8px] sm:text-[9px] md:text-xs lg:text-sm font-semibold uppercase tracking-tight"> <i class="fa-solid fa-wand-magic-sparkles"></i> Enhancement </span>`;
    }
    if (data.toLowerCase() === "documentation") {
      return `<span class="bg-blue-100 text-blue-600 border-blue-300 inline-flex items-center gap-0.5 sm:gap-1 md:gap-1.5 lg:gap-2 rounded-full border-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 lg:py-1.5 text-[8px] sm:text-[9px] md:text-xs lg:text-sm font-semibold uppercase tracking-tight"> <i class="fa-regular fa-file-lines"></i> Documentation </span>`;
    }
    if (data.toLowerCase() === "good first issue") {
      return `<span class="bg-purple-100 text-purple-600 border-purple-300 inline-flex items-center gap-0.5 sm:gap-1 md:gap-1.5 lg:gap-2 rounded-full border-2 px-2 sm:px-2.5 md:px-3 lg:px-4 py-1 lg:py-1.5 text-[8px] sm:text-[9px] md:text-xs lg:text-sm font-semibold uppercase tracking-tight"> <i class="fa-solid fa-circle-exclamation"></i> Good First Issue </span>`;
    }
  });
  return labels.join(" ");
};
...
const showIssueModal = async (issueId) => {
  ...
  modalContent.innerHTML = `
        ...
        <div class="modal-box w-11/12 sm:w-[85%] md:w-full sm:max-w-xl md:max-w-2xl lg:max-w-3xl rounded-lg md:rounded-2xl bg-[#F8FAFC] p-4 sm:p-5 md:p-7 lg:p-8 shadow-lg mx-auto">
            <h3 class="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-tight md:leading-snug text-gray-900"> ${issueData.title} </h3>
            <div class="mt-3 sm:mt-3.5 md:mt-4 lg:mt-5 flex flex-wrap items-center gap-2 sm:gap-2.5 text-xs sm:text-sm md:text-base lg:text-lg text-gray-600">
                <span class="rounded-full px-2.5 sm:px-3.5 md:px-4 lg:px-5 py-0.5 sm:py-0.5 md:py-1 lg:py-1.5 text-[10px] sm:text-xs md:text-sm lg:text-base font-semibold text-white ${issueData.status === "open" ? 'bg-green-500">Opened' : 'bg-red-500">Closed'}</span>
            ...
            <p class="mt-4 sm:mt-4.5 md:mt-6 lg:mt-7 text-xs sm:text-sm md:text-base lg:text-lg leading-relaxed md:leading-loose text-gray-600"> ${issueData.description} </p>
          
            <div class="mt-4 sm:mt-5 md:mt-6 lg:mt-8 rounded-lg md:rounded-xl lg:rounded-2xl bg-gray-100 p-3 sm:p-4 md:p-5 lg:p-6">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        <p class="text-[11px] sm:text-xs md:text-sm lg:text-lg text-gray-500 mb-1 md:mb-1.5 lg:mb-2 font-medium"> Assignee:</p>
                        <p class="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-800"> ${issueData.assignee.length === 0 ? "No One Assignee" : issueData.assignee} </p>
                    </div>

                    <div>
                        <p class="text-[11px] sm:text-xs md:text-sm lg:text-lg text-gray-500 mb-1 md:mb-1.5 lg:mb-2 font-medium"> Priority:</p>
            ...
            <div class="mt-4 sm:mt-4.5 md:mt-6 lg:mt-8 flex justify-end">
                <form method="dialog" class="w-auto">
                    <button class="btn w-auto h-auto border-0 bg-indigo-700 hover:bg-indigo-800 px-4 sm:px-5 md:px-6 lg:px-7 py-1.5 sm:py-2 md:py-2.5 text-xs sm:text-sm md:text-base lg:text-lg font-semibold text-white rounded-md md:rounded-lg lg:rounded-xl transition-all duration-200">Close</button>
                </form>
            </div>
        </div>
  `;
